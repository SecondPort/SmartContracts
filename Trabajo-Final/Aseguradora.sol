...
contract Laboratorio is OperacionesBasicas{
    ...
    function FuncionUnicamenteAsegurado(address _direccionAsegurado)
        public
        view
    {
        require(
            MappingAsegurados[_direccionAsegurado].AutorizacionCliente == true,
            "Direccion no autorizada."
        );
    }
    ...
    modifier UnicamenteAseguradora(address _direccionAsseguradora) {
        ...
        require(
            Aseguradora == _direccionAsseguradora,
            "Direccion no autorizada."
        );
        _;
    }
        ...
        UnicamenteAseguradora(msg.sender)
    {
        ...
        require(
            ServicioEstado(_nombreServicio) == true,
        ...
        returns (uint256 tokens)
    {
        ...
        require(
            MappingServicios[_nombreServicio].EstadoServicio == true,
    ...
    function compraTokens(address _asegurado, uint256 _numTokens)
        public
        payable
        UnicamenteAsegurado(_asegurado)
    {
...
contract InsuranceHealthRecord is OperacionesBasicas {
        ...
        Unicamente(msg.sender)
    {
        ...
        require(
            _numTokens > 0,
            "Necesitas comprar un numero de tokens positivo."
        );
        ...
        Unicamente(msg.sender)
    {
        ...
        require(
            ...
            "No tienes los tokens que desea devolver."
        );
        ...
        Unicamente(msg.sender)
    {
        ...
        require(
            InsuranceFactory(propietario.insurance).ServicioEstado(_servicio) ==
                true,
            "El servicio no se ha dado de alta."
        );
        // Se obtiene el precio del servicio a partir del otro contrato IF
        uint256 pagamientoTokens = InsuranceFactory(propietario.insurance)
            .getPrecioServicio(_servicio);
        // Es necesario que el precio del servicio sea menor al número de tokens de lo dispuesto
        require(
            pagamientoTokens <= balanceOf(),
            "Necesitas comprar más tokens para obtener este servicio"
        );
        ...
        Unicamente(msg.sender)
    {
        ...
        ServicioSolicitadoLab memory nuevoServicio = ServicioSolicitadoLab(
            _servicio,
    ...
    {
        return
            InsuranceFactory(propietario.insurance).consultarHistorialAsegurado(
                msg.sender,
                msg.sender
            );
    }
        ...
        returns (string memory nombreServicio, uint256 precioServicio)
    {
        return (
            ...
            historialAsegurado[_servicio].precioServicio
        );
    }
    ...
    function HistorialAseguradoLaboratorio()
        public
        view
        returns (ServicioSolicitadoLab[] memory)
    {
        return historialAseguradoLaboratorio;
    }

    // Funcion para devolver el estado del servicio (puede ser: true o false)
    function ServicioEstadoAsegurado(string memory _nombreServicio)
        public
        view
        returns (bool)
    {
...
contract Laboratorio is OperacionesBasicas {
    ...
    function ConsultarPrecioServicios(string memory _nombreServicio)
        public
        view
        returns (uint256)
    {
    ...
    function DarResultados(
        ...
        ResultadosServiciosLab[_direccionAsegurado] = ResultadoServicio(
            _diagnostico,
            _codigoIPFS
        );
    }
        ...
        returns (string memory _diagnostico, string memory _codigoIPFS)
    {
        ...
        _diagnostico = ResultadosServiciosLab[_direccionAsegurado]
